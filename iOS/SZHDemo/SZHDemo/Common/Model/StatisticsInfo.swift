...
//        let aSendLoss = "ASend Loss: MISSING%"
    ...
    func remoteDescription(info: RemoteInfo, audioOnly: Bool) -> String {
        ...
        let superResolution:String = "SR: \(info.superResolution ?? "")"
        if(audioOnly) {
            return [audioRecv,audioLoss,aquality].joined(separator: "\n")
        }
        return [dimensionFpsBit,videoRecv,audioRecv,videoLoss,audioLoss,aquality,superResolution].joined(separator: "\n")
    }
}
